(function () {
    var _this = this;
    if (!Config.BEHAVIOR_EDIT_MODE && !Config.EDIT_MODE && !Config.RELEASE_GAME) {
        EventUtils.addEventListener(ClientWorld, ClientWorld.EVENT_INITED, Callback.New(function () {
            if (Config.RELEASE_GAME)
                return;
            EventUtils.addEventListener(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED, Callback.New(function () {
                W26_varMonitor.completedLen += 1;
                if (W26_varMonitor.completedLen == 6) {
                    W26_varMonitor.start();
                    W26_varMonitor.allRecord();
                }
            }, _this));
            W26_varMonitor.loadJson();
        }, this));
    }
})();
var W26_varMonitor = (function () {
    function W26_varMonitor() {
    }
    W26_varMonitor.start = function () {
        W26_varMonitor.monitorVar = new Dictionary();
        stage.on(EventObject.KEY_DOWN, this, W26_varMonitor.onKeyDown);
    };
    W26_varMonitor.allRecord = function () {
        W26_varMonitor.addList(0, W26_varMonitor.variableString);
        W26_varMonitor.addList(1, W26_varMonitor.switchString);
        W26_varMonitor.addList(2, W26_varMonitor.stringString);
        W26_varMonitor.addList2(0, W26_varMonitor.variableString2);
        W26_varMonitor.addList2(1, W26_varMonitor.switchString2);
        W26_varMonitor.addList2(2, W26_varMonitor.stringString2);
    };
    W26_varMonitor.addList = function (varType, list) {
        var list2 = list["list"];
        var index1;
        for (var index in list2) {
            index1 = parseInt(index);
            for (var i = 0; i < list2[index].length; i++) {
                Game.player.addListenerPlayerVariable(varType, (index1 - 1) * 1000 + i, W26_varMonitor.recordFunction, false, false);
            }
        }
    };
    W26_varMonitor.addList2 = function (varType, list) {
        var list2 = list["list"];
        var index1;
        for (var index in list2) {
            index1 = parseInt(index);
            for (var i = 0; i < list2[index].length; i++) {
                ClientWorld.addListenerVariable(varType, (index1 - 1) * 1000 + i, W26_varMonitor.recordFunction2);
            }
        }
    };
    W26_varMonitor.addRecord = function (varType, globalVar, index, data) {
        var r = globalVar ? W26_varMonitor.records2 : W26_varMonitor.records;
        var ds = r.get(varType);
        if (ds == null) {
            r.set(varType, []);
            ds = r.get(varType);
        }
        if (ds[index] == null) {
            ds[index] = [];
        }
        ds[index].splice(0, 0, data);
        if (ds[index].length > WorldData.W26recordCount) {
            ds[index].pop();
        }
    };
    W26_varMonitor.getRecord = function (varType, globalVar, index) {
        var r = globalVar ? W26_varMonitor.records2 : W26_varMonitor.records;
        var ds = r.get(varType);
        if (ds == null)
            return [];
        if (ds[index] == null)
            return [];
        return ds[index];
    };
    W26_varMonitor.onKeyDown = function (e) {
        if (Config.RELEASE_GAME)
            return;
        if (!e.ctrlKey && !e.altKey && !e.shiftKey) {
            if ((WorldData.w26VarShowKey ? WorldData.w26VarShowKey : 120) == e.keyCode) {
                if (W26_varMonitor.UI == null) {
                    W26_varMonitor.UI = new W26_GUI_VarMonitor();
                    stage.addChild(W26_varMonitor.UI);
                }
                else {
                    W26_varMonitor.UI.visible = !W26_varMonitor.UI.visible;
                }
            }
            else if ((WorldData.w26VarShowMiniKey ? WorldData.w26VarShowMiniKey : 119) == e.keyCode) {
                if (W26_varMonitor.UIsimple == null) {
                    W26_varMonitor.UIsimple = new W26_GUI_MonitorVar();
                    stage.addChild(W26_varMonitor.UIsimple);
                }
                else {
                    W26_varMonitor.UIsimple.visible = !W26_varMonitor.UIsimple.visible;
                }
            }
        }
    };
    W26_varMonitor.loadJson = function () {
        AssetManager.loadJson("asset/json/server/variable/variable.json", Callback.New(function (jsonObj) {
            W26_varMonitor.variableString = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
        AssetManager.loadJson("asset/json/server/variable/string.json", Callback.New(function (jsonObj) {
            W26_varMonitor.stringString = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
        AssetManager.loadJson("asset/json/server/variable/switch.json", Callback.New(function (jsonObj) {
            W26_varMonitor.switchString = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
        AssetManager.loadJson("asset/json/variable/variable.json", Callback.New(function (jsonObj) {
            W26_varMonitor.variableString2 = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
        AssetManager.loadJson("asset/json/variable/string.json", Callback.New(function (jsonObj) {
            W26_varMonitor.stringString2 = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
        AssetManager.loadJson("asset/json/variable/switch.json", Callback.New(function (jsonObj) {
            W26_varMonitor.switchString2 = jsonObj;
            EventUtils.happen(W26_varMonitor, W26_varMonitor.LOAD_COMPLETED);
        }, this));
    };
    W26_varMonitor.addMonitorVar = function (varType, globalVar, index) {
        var data = W26_varMonitor.monitorVar.get(varType);
        var data2;
        if (data == null) {
            data = new Dictionary();
            data.set(true, []);
            data.set(false, []);
            W26_varMonitor.monitorVar.set(varType, data);
        }
        data2 = data.get(globalVar);
        if (data2.indexOf(index) != -1) {
            return;
        }
        data2.push(index);
        if (W26_varMonitor.UIsimple == null) {
            W26_varMonitor.UIsimple = new W26_GUI_MonitorVar();
            stage.addChild(W26_varMonitor.UIsimple);
        }
        W26_varMonitor.UIsimple.creatorMonitorVar(varType, globalVar, index);
    };
    W26_varMonitor.removeMonitorVar = function (varType, globalVar, index) {
        var data = W26_varMonitor.monitorVar.get(varType);
        var data2;
        if (data == null)
            return;
        data2 = data.get(globalVar);
        if (data2 == null)
            return;
        var i = data2.indexOf(index);
        if (i == -1)
            return;
        if (W26_varMonitor.UIsimple == null) {
            W26_varMonitor.UIsimple = new W26_GUI_MonitorVar();
            stage.addChild(W26_varMonitor.UIsimple);
        }
        W26_varMonitor.UIsimple.removeMonitorVar(varType, globalVar, data2[i]);
        data2.splice(i, 1);
        W26_varMonitor.UI.updateTraceState();
    };
    W26_varMonitor.setMode = function (index, mode) {
        W26_varMonitor.modeType.set(index, mode);
    };
    W26_varMonitor.getMode = function (index) {
        return W26_varMonitor.modeType.get(index);
    };
    W26_varMonitor.checkArrayPlugin = function () {
        try {
            W26_VisualArray;
            return true;
        }
        catch (_b) {
            return false;
        }
    };
    W26_varMonitor.checkStructPlugin = function () {
        try {
            W26_Struct;
            return true;
        }
        catch (_b) {
            return false;
        }
    };
    W26_varMonitor.getStructPluginV = function () {
        if (this.checkStructPlugin()) {
            if (W26_Struct["version"]) {
                return W26_Struct["version"];
            }
            return 0;
        }
        return 0;
    };
    W26_varMonitor.getArrayPluginV = function () {
        if (this.checkArrayPlugin()) {
            if (W26_VisualArray["version"]) {
                return W26_VisualArray["version"];
            }
            return 0;
        }
        return 0;
    };
    W26_varMonitor.checkPlugins = function () {
        return this.checkArrayPlugin() || (this.checkStructPlugin() && this.getStructPluginV() == 2.0);
    };
    W26_varMonitor.getPluginsV = function () {
        return this.getStructPluginV() == 2.0 || this.getArrayPluginV() == 4.0;
    };
    W26_varMonitor.getVarName = function (varType, globalVar, index) {
        var data;
        if (globalVar) {
            if (varType == 0) {
                data = W26_varMonitor.variableString2["list"];
            }
            else if (varType == 1) {
                data = W26_varMonitor.switchString2["list"];
            }
            else if (varType == 2) {
                data = W26_varMonitor.stringString2["list"];
            }
        }
        else {
            if (varType == 0) {
                data = W26_varMonitor.variableString["list"];
            }
            else if (varType == 1) {
                data = W26_varMonitor.switchString["list"];
            }
            else if (varType == 2) {
                data = W26_varMonitor.stringString["list"];
            }
        }
        var page = Math.floor(index / 1000) + 1;
        var varNames = data[page.toString()];
        if (varNames == null)
            return "";
        var result = varNames[(index % 1000)];
        if (result == null)
            return "";
        return result;
    };
    var _a;
    _a = W26_varMonitor;
    W26_varMonitor.LOAD_COMPLETED = "LOAD_COMPLETED";
    W26_varMonitor.completedLen = 0;
    W26_varMonitor.modeType = new Dictionary();
    W26_varMonitor.records = new Dictionary();
    W26_varMonitor.records2 = new Dictionary();
    W26_varMonitor.recordFunction = Callback.New(function (typeID, varID, value) {
        W26_varMonitor.addRecord(typeID, false, varID, value);
    }, _a);
    W26_varMonitor.recordFunction2 = Callback.New(function (typeID, varID, value) {
        W26_varMonitor.addRecord(typeID, true, varID, value);
    }, _a);
    return W26_varMonitor;
}());
//# sourceMappingURL=W26_varMonitor.js.map